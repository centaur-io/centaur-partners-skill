...
`list_aggregate_summaries` — Generated Aggregate Narrative Summaries across eligible sources. `list_channel_summaries` — compact Generated Channel Narrative Summaries. Time filters on both select windows that overlap the requested interval.
...
`rank_traders` — accepts `metric` (`event_count` default, `position_count`, `win_rate`, `avg_return`, `median_return`, `sharpe_ratio`), optional `assetIds`, `direction`, `timeBasedPerformanceWindow` (default `7D`), `minPositions` (default `3`), `startTime`, `endTime` (default last 7 days), and `limit` (default `10`, max `50`). Results are bounded with no cursor; `meta.totalCandidates` reports how many traders qualified. Performance metrics evaluate positions opened in the window at the chosen horizon — a position opened at time T is evaluated at T plus the window — so `startTime` must be at least the evaluation window before now (a `30D` ranking needs `startTime` at least 30 days in the past). More recent windows are rejected with a corrective error: widen the window start or use a shorter `timeBasedPerformanceWindow` and retry.
